...
import { TrendingUp, Bell, MessageSquare, Play, ArrowLeft } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background-main py-12 px-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-4xl font-bold text-text-dark mb-2">AI Features</h1>
            <p className="text-text-muted">
              AI-powered forecasting, alerts, and CFO chat assistant
            </p>
          </div>
          <Link
            href="/dashboard"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((feature, index) => {
            const Icon = iconMap[feature.id] || TrendingUp;
            return (
              <Link
                key={feature.id}
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center space-x-3">
                      <div className="w-12 h-12 rounded-lg bg-primary-teal/10 flex items-center justify-center group-hover:bg-primary-teal/20 group-hover:scale-110 transition-all">
                        <Icon className="h-6 w-6 text-primary-teal" />
                      </div>
                      <div>
                        <h3 className="font-semibold text-text-dark group-hover:text-primary-teal transition-colors">{feature.title}</h3>
                  ...
                  <div className="w-full px-4 py-2 text-sm bg-primary-teal text-white rounded hover:bg-primary-teal/90 text-center transition-colors inline-flex items-center justify-center space-x-2 group-hover:shadow-lg">
                    <span>Open</span>
